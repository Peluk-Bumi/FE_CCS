...
import { footerContactItems } from "@/app/config/footerContactConfig";
...
export default function Footer() {
  ...
  return (
    <footer
      className={`py-12 px-4 sm:px-6 max-md:pb-28 ${isDark ? 'bg-gray-900' : 'bg-gray-100'
        }`}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-6 mb-6">
            ...
            >
              ...
              <p className={`text-sm leading-relaxed mb-4 text-balance ${isDark ? 'text-gray-400' : 'text-gray-600'
                }`}>
              ...
              <ul className="grid grid-cols-2 md:grid-cols-1 gap-2">
                {footerLinks.map((link, index) => (
                  ...
                  >
                    <motion.a
                      href={link.href}
                      className={`text-sm transition-colors hover:text-primary inline-block relative ${isDark ? 'text-gray-400' : 'text-gray-600'
                        }`}
                      whileHover={{ x: 5, y: -2 }}
                      whileTap={{ scale: 0.95 }}
                      transition={{ duration: 0.2 }}
                ...
                {footerContactItems.map((item) => {
                  ...
                  return (
                    <motion.a
                      ...
                      className={`flex items-center gap-2 p-1.5 rounded-lg transition-colors hover:bg-primary/10 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
                    ...
                    >
                      <motion.div
                        ...
                        whileHover={{ rotate: item.id === 'instagram' ? 15 : item.id === 'email' ? 0 : -15 }}
                        transition={{ type: "spring", stiffness: 300 }}
                      >
                        <Icon className={`w-4 h-4 ${isDark ? 'text-white' : 'text-gray-900'}`} />
                      </motion.div>
                      <span className={item.id === 'maps' ? "text-sm text-balance" : "text-sm"}>{item.label}</span>
                    </motion.a>
                  );
                })}
              </div>
          ...
          >
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 w-full">
              <p className={`text-sm font-bold ${isDark ? 'text-gray-500' : 'text-gray-500'
                }`}>
              ...
              >
                {['kebijakan-privasi', 'syarat-ketentuan', 'lisensi'].map((link, index) => (
                  <motion.a
                    key={link}
                    href={`/${link}`}
                    className={`transition-colors hover:text-primary px-2 ${isDark ? 'text-gray-400' : 'text-gray-600'
                      }`}
                    initial={{ opacity: 0, y: 10 }}
                    whileInView={{ opacity: 1, y: 0 }}
